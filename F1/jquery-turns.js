$(document).ready(function(){
  ...
  index=0
  check=true
  timer=setInterval(lunbo,3000)  //3s定时   //调用说不用加引号 可是加了也没错。。
/*鼠标左右切换效果 ps...  */
...
function hiden()
{
  $(".pic").eq(index).css("opacity","1").fadeIn(500);  //将所选择的index透明度置1  在此可以添加淡入淡出;
  $("ul#num>li").eq(index).css("background-color","rgba(166, 166, 166,0.9)");
  for(i=0;i<6;i++)
  {
    if(i!=index)           //不知道为什么for循环里面条件不能相与？？？？？
      {
         $("ul#num>li").eq(i).css("background-color","rgba(166, 166, 166,0.2)");
         $(".pic").eq(i).css("opacity","0").fadeOut(500);   //将所选择的index透明度置1  在此可以添加淡入淡出;
       }
  }
}
function lunbo()
